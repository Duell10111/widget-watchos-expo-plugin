...
async function addXcodeTarget(
    ...
    if (target.type === "complication") {
        ...
        xcodeProject.addTargetDependency(targets[watchAppIndex], [
            newTarget.uuid,
        ]);
        ...
        // into the watch app
        const primaryTargetBuildPhase = xcodeProject.pbxCopyfilesBuildPhaseObj(
            targets[0]
        );
    ...
    if (target.entitlementsFile) {
        ...
        ] = `${target.name}/${target.entitlementsFile}`;
    }
...
function getTargetUuids(project: any) {
    ...
    return project
        .getFirstProject()
        ["firstProject"]["targets"].map((t: any) => t.value);
}
